...
- **Agent 생성**: 환경에 따라 달라짐. 측정 명령, 환경과 할당 범위는 [Performance](/ko/advanced/performance) 참고
- **메모리**: `B/op`는 Agent 전체 상주 메모리가 아니라 benchmark 할당 통계
- **동시 Agent**: 고정 수치가 아니라 배포 부하와 리소스에 따라 한도를 정함
